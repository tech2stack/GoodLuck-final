...
const TransportManagement = ({ showFlashMessage }) => {
  ...
  // ✅ highlight + scroll to top
  const [highlightedId, setHighlightedId] = useState(null);
  const topRef = useRef(null);

  useEffect(() => {
    if (highlightedId && topRef.current) {
    ...
    try {
      ...
      if (editingTransportId) {
        response = await api.patch(
          `/transports/${editingTransportId}`,
          formData
        );
        if (response.data.status === 'success') {
          showFlashMessage('Transport updated successfully!', 'success');
          fetchTransports();
          setHighlightedId(editingTransportId);
          setTimeout(() => setHighlightedId(null), 10000);
        } else {
          throw new Error(
            response.data.message || 'Failed to update transport.'
          );
        }
      } else {
        ...
        if (response.data.status === 'success') {
          ...
          const newTransport = response.data.data.transport;
          setTransports((prev) => [newTransport, ...prev]);
          setCurrentPage(1);
          setHighlightedId(newTransport._id);
          setTimeout(() => setHighlightedId(null), 10000);
        } else {
          throw new Error(
            response.data.message || 'Failed to create transport.'
          );
        }
      }
  ...
  const downloadPdf = () => {
    if (
      ...
      showFlashMessage(
        'PDF generation failed: Core libraries are not loaded. Please check your script links.',
        'error'
      );
      console.error(
        'PDF generation failed: window.jspdf is not available. Ensure CDNs for jsPDF are correctly linked in your HTML file.'
      );
      return;
    }
  ...
  return (
    <div className="transport-management-container">
      <h2 ref={topRef} className="main-section-title">
        ...
        <div className="table-container">
          <div className="table-controls">
            ...
            <button
              ...
              disabled={loading || filteredTransports.length === 0}
            >
          ...
          {loading && transports.length === 0 ? (
            ...
            <p className="no-data-message text-center">
              No transports found matching your criteria. Start by adding one!
            </p>
          ) : (
            <>
                ...
                <tbody ref={tableBodyRef}>
                  {currentTransports.map((transport, index) => (
                    <tr
                      key={transport._id}
                      className={
                        highlightedId === transport._id ? 'highlighted-row' : ''
                      }
                    >
                      <td>
                      ...
                      <td className="actions-column">
                        ...
                        >
                          {loading &&
                          transportToDeleteId === transport._id ? (
        ...
        <div className="modal-backdrop">
          <div className="modal-content">
            ...
            <p>
              ...
              <strong>{transportToDeleteName}</strong>?
            </p>
